...
// DELETE deletes the question and its answers by id
...
async function getRandomQuestion() {
  ...
  let answers = await queryDb(`SELECT * FROM answers WHERE question_id = ?`, [
    question.id,
  ]);
...
async function addQuestion(question, answers) {
  ...
  let insert = await queryDb("INSERT INTO questions (question) VALUES (?)", [
    question,
  ]);
  ...
  for (let i = 0; i < answers.length; i++) {
    await queryDb(
      ...
      [questionId, arrAnswers[i], arrIsCorrect[i]]
    );
  };
  return await queryDb(
    `SELECT question, question_id, answer FROM questions JOIN answers ON questions.id = question_id WHERE questions.id = ? AND is_correct = ?`,
    [questionId, 1]
  );
};
